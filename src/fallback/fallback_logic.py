...
class FallbackManager:
    ...
    def run(self, request: deep_client.ChatRequest, prompt: deep_client.Message) -> dict[str, str]:
        ...
        try:
            ...
            # Конвертируем в формат fallback
            custom_response = {
                "title": custom_response.get("title", ""),
                "date": custom_response.get("date", ""),
                "time": custom_response.get("time", ""),
                "loc": custom_response.get("location", ""),
                "user": ", ".join(custom_response.get("participants", [])),
                "url": custom_response.get("url", ""),
            }
        except Exception as e:
            print(f"[FallbackManager] Ошибка модели: {e}")
            custom_response = {}
