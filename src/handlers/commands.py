...
async def choose_date(callback: types.CallbackQuery, state: FSMContext) -> None:
    if callback.data is None or callback.message is None:
        return
...
async def choose_hour(callback: types.CallbackQuery, state: FSMContext) -> None:
    if callback.data is None or callback.message is None:
        return
...
# --- /Выбор минут ---
@router.callback_query(F.data.startswith("time_") & F.data.contains(":"))
async def choose_minute(callback: types.CallbackQuery, state: FSMContext) -> None:
    if callback.data is None or callback.message is None:
        return
    ...
    await callback.message.edit_text(
        f"Вы выбрали время {hour:02d}:{minute:02d}. " f"Теперь выберите длительность:",
...
async def confirm_event(callback: types.CallbackQuery, state: FSMContext) -> None:
    if callback.data is None or callback.message is None:
        return
...
async def go_back(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if current_state is None:
        ...
        await callback.message.edit_text(
            "Выберите режим события:", reply_markup=get_mode_keyboard()
        )

    elif current_state.endswith("choosing_hour"):
        await state.set_state(EventStates.choosing_date)
        await callback.message.edit_text(
            "Выберите дату события:", reply_markup=get_calendar_keyboard()
        )

    elif current_state.endswith("choosing_minute"):
        await state.set_state(EventStates.choosing_hour)
        await callback.message.edit_text(
            "Выберите час:", reply_markup=get_time_hour_keyboard()
        )
        ...
        data = await state.get_data()
        await callback.message.edit_text(
            "Выберите минуты:", reply_markup=get_time_minute_keyboard(data["hour"])
        )
        ...
        data = await state.get_data()
        await callback.message.edit_text(
            "Выберите длительность:", reply_markup=get_duration_keyboard()
        )
