...
async def echo_all(message: Message) -> None:
    ...
    await message.answer(
        LEXICON_RU["echo_message"].format(text=text, user_id=user.id, length=len(text))
    )
...
async def hello_button(message: Message) -> None:
    ...
    await message.answer(LEXICON_RU["hello_user"].format(name=user.first_name))
...
async def random_button(message: Message) -> None:
    ...
    await message.answer(LEXICON_RU["random_number"].format(number=number))
