...
def get_calendar_keyboard(
    ...
    for week in month_days:
        ...
        for day in week:
            if day == 0:
                ...
                row.append(
                    InlineKeyboardButton(
                        text=str(day), callback_data=f"date_{year}_{month}_{day}"
                    )
                )
        keyboard.append(row)
    ...
    next_month = datetime(year, month, calendar.monthrange(year, month)[1]) + timedelta(
        days=1
    )
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                text="⬅️", callback_data=f"month_{prev_month.year}_{prev_month.month}"
            ),
            InlineKeyboardButton(
                text="➡️", callback_data=f"month_{next_month.year}_{next_month.month}"
            ),
        ]
    )
...
def get_time_hour_keyboard() -> InlineKeyboardMarkup:
    ...
    for hour in range(24):
        row.append(InlineKeyboardButton(text=str(hour), callback_data=f"time_{hour}"))
        if (hour + 1) % 6 == 0:
            keyboard.append(row)
            row = []
    if row:
...
def get_time_minute_keyboard(hour: int) -> InlineKeyboardMarkup:
    ...
    for minute in range(0, 60, 5):
        row.append(
            InlineKeyboardButton(
                text=f"{minute:02}", callback_data=f"time_{hour}:{minute}"
            )
        )
        if (len(row)) % 6 == 0:
            keyboard.append(row)
...
def get_duration_keyboard() -> InlineKeyboardMarkup:
    ...
    for d in durations:
        keyboard.append(
            [InlineKeyboardButton(text=f"{d} мин", callback_data=f"duration_{d}")]
        )
...
def get_auth_keyboard(server_url: str, telegram_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="🔐 Авторизоваться через Google",
                    url=f"{server_url}/auth/google?user_id={telegram_id}",
                )
            ]
        ]
    )
